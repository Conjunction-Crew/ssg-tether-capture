...
use bevy::light::{CascadeShadowConfigBuilder, GeneratedEnvironmentMapLight, SunDisk};
use bevy::math::cubic_splines::LinearSpline;
use bevy::pbr::{
    Atmosphere, AtmosphereMode, AtmosphereSettings, ScatteringMedium, ScreenSpaceReflections,
};
    ...
    commands
        .spawn((
            ...
            GeneratedEnvironmentMapLight {
                environment_map: skybox_handle.clone(),
                intensity: 1000.0,
                ..default()
            },
...
pub fn tune_satellite_material(
    mut materials: ResMut<Assets<StandardMaterial>>,
    asset_server: Res<AssetServer>,
    mut tuned_material: Local<bool>,
    mut tuned_default: Local<bool>,
) {
    if *tuned_material && *tuned_default {
        return;
    }
    ...
    if !*tuned_material {
        let material: Handle<StandardMaterial> = asset_server.load(
            ...
            .from_asset("models/broken_satellite.glb"),
        );

        if let Some(mat) = materials.get_mut(&material) {
            mat.metallic = 1.0;
            mat.perceptual_roughness = 0.02;
            mat.reflectance = 1.0;
            *tuned_material = true;
        }
    }
    ...
    if !*tuned_default {
        let default_material: Handle<StandardMaterial> =
            asset_server.load(GltfAssetLabel::DefaultMaterial.from_asset("models/broken_satellite.glb"));

        if let Some(mat) = materials.get_mut(&default_material) {
            mat.metallic = 1.0;
            mat.perceptual_roughness = 0.02;
            mat.reflectance = 1.0;
            *tuned_default = true;
