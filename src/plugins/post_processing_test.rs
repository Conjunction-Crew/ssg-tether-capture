...
const SHADER_ASSET_PATH: &str = "shaders/post_processing_test.wgsl";
...
impl ViewNode for PostProcessNode {
    ...
    fn run(
        ...
        // Match the pipeline target format to the current view's main texture format.
        let pipeline_id = if view_target.is_hdr() {
            post_process_pipeline.pipeline_id_hdr
        } else {
            post_process_pipeline.pipeline_id_ldr
        };
...
fn init_post_process_pipeline(
    ...
    let queue_pipeline = |format: TextureFormat, label: &str| {
    ...
    let pipeline_id_ldr = queue_pipeline(TextureFormat::bevy_default(), "post_process_pipeline_ldr");
    let pipeline_id_hdr = queue_pipeline(ViewTarget::TEXTURE_FORMAT_HDR, "post_process_pipeline_hdr");
